...
public class Match {
    ...
    private void afterGameMaybeEndSet() {
        if (isTieBreakSetWon()) {
            int setWinner = (gamesA > gamesB) ? 0 : 1;
            awardSetTo(setWinner);
            return;
        }
    ...
    // ── gettery potrzebne w repo/serwisach ──────────────────────
     ...
     */
    public Match copy() {
        Match c = new Match(this.id, this.playerA, this.playerB, this.rules);
    ...
    /**
     * Przywrócenie stanu z DTO (używa repo).
     */
